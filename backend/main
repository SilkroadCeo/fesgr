...
telegram_sessions = {}  # {session_id: {user_data, created_at}}
...
async def telegram_auth(request: Request, response: Response):
    ...
    try:
        ...
        response.set_cookie(
            ...
            samesite="none",  # Required for cross-origin cookies (ngrok, Telegram WebApp)
...
async def get_chat_messages(profile_id: int, user: dict = Depends(get_telegram_user)):
    ...
    USER ISOLATION: Требуется авторизация. Возвращает сообщения только из чатов пользователя.
    """
    data = load_data()
...
async def get_chat_updates(profile_id: int, last_message_id: int = 0, user: dict = Depends(get_telegram_user)):
    ...
    USER ISOLATION: Требуется авторизация. Возвращает обновления только из чатов пользователя.
    """
    data = load_data()
...
async def get_user_chats(user: dict = Depends(get_telegram_user)):
    """
    Получить список всех чатов пользователя с последним сообщением и счетчиком непрочитанных

    USER ISOLATION: Требуется авторизация. Возвращает только чаты этого пользователя.
    """
    data = load_data()
...
async def mark_chat_read(profile_id: int, user: dict = Depends(get_telegram_user)):
    ...
    USER ISOLATION: Требуется авторизация. Работает только с чатами пользователя.
    """
    data = load_data()
